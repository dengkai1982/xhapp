...
<script type="text/javascript">
    ...
    function createMenuItems(dataId,dataRow,data){
        var items = [{
            url:"${managerPath}/sys/consultation/detail${suffix}?entityId="+dataId+"&${paginationCurrentPage}="+getPaginationCurrentPage(),
            label:"查看详情"
        }]
        if(data.answer.name=="false"){
            items.push({
                url:"${managerPath}/sys/consultation/reply${suffix}?entityId="+dataId+"&${paginationCurrentPage}="+getPaginationCurrentPage(),
                label:"留言回复",
                className:"privilege",
                access:"/mgr/sys/consultation/reply"
            });
        }
