...
<script type="text/javascript">
    ...
    function createMenuItems(dataId,dataRow,data){
        var items = [{
            url:"${managerPath}/curriculum/faceToFace/detail${suffix}?entityId="+dataId+"&${paginationCurrentPage}="+getPaginationCurrentPage(),
            label:"查看详情"
        }];
        if(data.answer.name=="false"){
            items.push({
                url:"${managerPath}/curriculum/faceToFace/reply${suffix}?entityId="+dataId+"&${paginationCurrentPage}="+getPaginationCurrentPage(),
                label:"面授预约回复",
                className:"privilege",
                access:"/mgr/curriculum/faceToFace/reply"
            });
        }
