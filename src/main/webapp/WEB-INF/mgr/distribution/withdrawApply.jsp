        ...
        <div id="mainMenu" class="clearfix">
            <div class="btn-toolbar pull-right">
                <c:if test="${requestScope.hasData}">
                    <a href="#" class="btn btn-link querybox-toggle" id="show_or_hide_search"><i class="icon icon-search"></i> 搜索</a>
                    <a href='javascript:void(0)' id="exportTableToExcel" title="提现信息表" class="btn btn-link"><i class="icon-import muted"> </i> 导出数据表</a>
                </c:if>
            </div>
        </div>
        <c:if test="${requestScope.hasData}">
...
<script type="text/javascript">
    ...
    function createMenuItems(dataId,dataRow,data){
        ...
        console.log(data);
        if(data.status.name=="WAIT_VERIFY"){
            items.push({
