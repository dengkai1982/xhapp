        ...
        <div id="mainMenu" class="clearfix">
            <div class="btn-toolbar pull-right">
                <c:if test="${requestScope.hasData}">
                    ...
                    <a href='javascript:void(0)' id="exportTableToExcel" title="资金流水记录" class="btn btn-link"><i class="icon-import muted"> </i> 导出数据表</a>
                </c:if>
            </div>
        </div>
        <c:if test="${requestScope.hasData}">
...
<script type="text/javascript">
    ...
    function dataLoadSuccess(query){
        ...
        postJSON("${managerPath}/statistics/memberAmountFlowCount${suffix}", searchForm, "正在处理请稍后", function (result) {
            var flowStatistics=result.flowStatistics;
            $(".extar_oper_container").html("合计发生额:<span class='text-red'>"+flowStatistics.totalAmount+"</span>");
        })
    }
</script>
</body>
