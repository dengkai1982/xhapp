        ...
        <c:if test="${requestScope.hasData}">
            <div id="queryBox" class="cell hidden">
                <form class="form-horizontal" id="query_search_from" method="post">
                    <input type="hidden" name="serviceName" value="noticeService"/>
...
<script type="text/javascript">
    ...
    function createMenuItems(dataId,dataRow,data){
        var items = [{
            ...
            onClick:function(){
                confirmOper("警告","确实要删除选中的公告?",function(){
                    postJSON("${managerPath}/public/notice/delete${suffix}",{
                        entityId:dataId
                    },"正在执行,请稍后...",function(result){
                        if(result.code==SUCCESS){
                            bootbox.alert({
                                title:"消息",
                                message: "删除公告成功,点击确认返回",
                                callback: function () {
                                    reflashPageData();
                                }
                            })
